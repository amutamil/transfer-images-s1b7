...
{
    char            ServiceName[50]; /* MergeCOM-3 Service Name. May be zero length if the SOP Class is not known by MergeCOM-3 */
    char            SOPClassUID[66]; /* Service Class UID for the service if defined in the association request */
} RelatedSOPClass;
...
{
    char            ServiceName[50];        /* MergeCOM-3 Service Name.  May be zero length if this context
                                               was accepted because of a related general SOP Class that we do support */
    TRANSFER_SYNTAX SyntaxType;             /* Transfer syntax negotiated for the service */
    ROLE_TYPE       RoleNegotiated;         /* The role negotiated for the service */
    int             PresentationContextID;  /* Presentation Context ID */
    char            SOPClassUID[66];        /* SOP Class UID associated with ServiceName */
    char            ServiceClassUID[66];    /* Optional Service Class UID for the service if defined in the association request */
    int             NumberRelatedSOPClasses;/* Number of related general SOP Classes contained in the association request */
    RelatedSOPClass RelatedSOPClasses[10];  /* Array containing related general SOP classes for this negotiated serivce */
} ServiceInfo;
 ...
 *   specify the reason why the association is being rejected.              */
typedef enum {
        PERMANENT_NO_REASON_GIVEN,                          /* result = 2, source = 1, reason = 1 */
        PERMANENT_NO_REASON_GIVEN_SERV_USER = PERMANENT_NO_REASON_GIVEN,
        PERMANENT_NO_REASON_GIVEN_SERV_PROV_ACSE,           /* 1, 2, 1 */

        TRANSIENT_NO_REASON_GIVEN,                          /* 2, 1, 1 */
        TRANSIENT_NO_REASON_GIVEN_SERV_USER = TRANSIENT_NO_REASON_GIVEN,
        TRANSIENT_NO_REASON_GIVEN_SERV_PROV_ACSE,           /* 2, 2, 1 */ 

        PERMANENT_APPLICATION_CONTEXT_NAME_NOT_SUPPORTED,   /* 1, 1, 2 */
        PERMANENT_CALLING_AE_TITLE_NOT_RECOGNIZED,          /* 1, 1, 3 */
        PERMANENT_CALLED_AE_TITLE_NOT_RECOGNIZED,           /* 1, 1, 7 */
        PERMANENT_CALLING_AE_QUALIFIER_NOT_RECOGNIZED,      /* 1, 1, 8  - not in the DICOM Standard */
        PERMANENT_CALLING_AP_INVOCATION_ID_NOT_RECOGNIZED,  /* 1, 1, 9  - not in the DICOM Standard */
        PERMANENT_CALLING_AE_INVOCATION_ID_NOT_RECOGNIZED,  /* 1, 1, 10 - not in the DICOM Standard */
        PERMANENT_CALLED_AE_QUALIFIER_NOT_RECOGNIZED,       /* 1, 1, 11 - not in the DICOM Standard */
        PERMANENT_CALLED_AP_INVOCATION_ID_NOT_RECOGNIZED,   /* 1, 1, 12 - not in the DICOM Standard */
        PERMANENT_CALLED_AE_INVOCATION_ID_NOT_RECOGNIZED,   /* 1, 1, 13 - not in the DICOM Standard */

        PERMANENT_ACSE_PROTOCOL_VERSION_NOT_SUPPORTED,      /* 1, 2, 2 */

        TRANSIENT_TEMPORARY_CONGESTION,                     /* 2, 3, 1 */
        TRANSIENT_LOCAL_LIMIT_EXCEEDED,                     /* 2, 3 ,2 */

        PERMANENT_CALLED_PRES_ADDRESS_UNKNOWN,              /* 1, 3, 3 - not in the DICOM Standard */
        PERMANENT_PRES_PROTOCOL_VERSION_NOT_SUPPORTED,      /* 1, 3, 4 - not in the DICOM Standard */
        PERMANENT_NO_PRESENTATION_SAP_AVAILABLE,            /* 1, 3, 5 - not in the DICOM Standard */
        NUM_REJECT_REASONS
} REJECT_REASON;

/* ======================================================================== *
 *              Association rejection codes structure                     *
 * ======================================================================== */
typedef struct MC_Assoc_Reject_Info {
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Association(int         ApplicationID,
                                                      ...
                                                      char*       ServiceList);
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Association_With_Reject_Info(int             ApplicationID,
                                                                       int*            AssociationID,
                                                                       const char*     RemoteApplicationTitle,
                                                                       int*            RemoteHostPortNumber,
                                                                       char*           RemoteHostTCPIPName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Association_With_Socket(int         ApplicationID,
                                                                  int*        AssociationID,
                                                                  const char* RemoteApplicationTitle,
                                                                  int*        RemoteHostPortNumber,
                                                                  char*       RemoteHostTCPIPName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Secure_Association_With_Reject_Info(int                    ApplicationID,
                                                                              int*                   AssociationID,
                                                                              const char*            RemoteApplicationTitle,
                                                                              int*                   RemoteHostPortNumber,
                                                                              char*                  RemoteHostTCPIPName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Secure_Association_With_Socket(int                    ApplicationID,
                                                                         int*                   AssociationID,
                                                                         const char*            RemoteApplicationTitle,
                                                                         int*                   RemoteHostPortNumber,
                                                                         char*                  RemoteHostTCPIPName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Association_With_Identity_With_Reject_Info(int                    ApplicationID,
                                                                                     int*                   AsessionID,
                                                                                     const char*            AremoteAppTitle,
                                                                                     int*                   AremotePortNumber,
                                                                                     char*                  AremoteHostName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Open_Association_With_Identity_With_Socket(int                    ApplicationID,
                                                                                int*                   AsessionID,
                                                                                const char*            AremoteAppTitle,
                                                                                int*                   AremotePortNumber,
                                                                                char*                  AremoteHostName,
...
EXP_PRE extern MC_STATUS EXP_FUNC MC_Reject_Association(int AassociationID, REJECT_REASON Areason);
EXP_PRE extern MC_STATUS EXP_FUNC MC_Reject_Association_With_Reason_Codes (int AassociationID,
                                                                           unsigned short Aresult,
                                                                           unsigned short Asource,
